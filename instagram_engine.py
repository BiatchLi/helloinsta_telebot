...
def get_id(username):
    address = 'https://www.instapi.io/u/{}'.format(username)
    req = requests.get(address, headers)
    data = json.loads(req.text)
    # print(data['graphql']['user']['id'])
    print ("User ID: ", data['graphql']['user']['id'])
    return str(data['graphql']['user']['id'])

def get_post_owner(shortcode):
    address = 'https://www.instagram.com/p/{}/?__a=1'.format(shortcode)
    req = requests.get(address, headers)
    data = json.loads(req.text)
...
def get_followers(username):
    ...
    # print(req.text)
    data = json.loads(req.text)
    print ("Followers: ", data['graphql']['user']['edge_followed_by']['count'])
    return int(data['graphql']['user']['edge_followed_by']['count'])
...
def get_likes(shortcode):
    try:
        if shortcode != "":
            ...
            print(address)
            data = json.loads(req.text)
            print ("Likes: ", data['graphql']['shortcode_media']['edge_media_preview_like']['count'])
            likes = data['graphql']['shortcode_media']['edge_media_preview_like']['edges']
            print ("Likes: ", likes)
            # commented bcz change in the insta api it wont let u retrive the list of people who liked the post
            #  
            # try:
            #     if likes > 3000:
            #         count = likes
        ...
        if shortcode != "":
            ...
            #             return COMMENTS_LIST
                    ...
                    for i in range(comments):                    
                        try:
                            COMMENTS_LIST.append(str(data['graphql']['shortcode_media']['edge_media_to_comment']['edges'][i]['node']['owner']['id']))
                        except Exception:
                            continue
                    return COMMENTS_LIST
                elif comments == 1:
                    COMMENTS_LIST.append(str(data['graphql']['shortcode_media']['edge_media_to_comment']['edges'][0]['node']['owner']['id']))
                    return COMMENTS_LIST
            except Exception as e:
                print (e)
                pass
